...
# Panel 1 – 3D median price map with time slider
...
def run_panel():
    ...
    counties = gpd.read_file(
        DATA_DIR / "Administrative_Areas___OSi_National_Statutory_Boundaries_-978765968693620525.geojson"
    )
    counties = counties.to_crs(epsg=2157)

    # --- Filter to median price ---
    df_median = df[
                ...
                layout=dict(
                    title=dict(
                        text=(
                            ...
                            f"{d.strftime('%B %Y')}<br>"
                            "<sup>Source: Central Statistics Office (CSO), Table HPM03</sup>"
                        )
                    )
                ),
            )
    ...
    # --- Layout tuned for dashboard cell ---
    ...
    color="#3C3C3C"
        ...
        title=dict(
            text=(
                "3D Visualisation of Median Price by Region - "
                f"{period.strftime('%B %Y')}<br>"
                "<sup>Source: Central Statistics Office (CSO), Table HPM03 -https://data.cso.ie/ </sup>"
            ),
            font=dict(
    family="Georgia, 'Times New Roman', serif",
    size=14,
